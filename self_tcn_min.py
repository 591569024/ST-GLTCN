...
parser.add_argument('--len_local', type=int, default=2, help='the length of local time')
parser.add_argument('--neighbor_size', type=int, default=3, help='the size of neighbor size')
parser.add_argument('--num_tcn', type=int, default=3, help='how many tcn')
parser.add_argument('--nb_stacks', type=int, default=2, help='how many res unit in a tcn')
...
# model params
param_dict['model'] = ['TCN', 'simple_tcn',
                       {'len_local': args.len_local, 'neighbor_size': args.neighbor_size,
                        'num_tcn': args.num_tcn, 'nb_stacks': args.nb_stacks, 'dataset': param_dict['dataset']}]
...
# file params
param_dict['header'] = {'len_local': 10, 'neighbor_size': 15, 'num_tcn': 10, 'nb_stacks': 10,
...
param_dict['hyper_params'] = {'len_local': args.len_local, 'neighbor_size': args.neighbor_size,
                             'num_tcn': args.num_tcn, 'nb_stacks': args.nb_stacks, 'lr': args.lr}
...
if __name__ == '__main__':
    ...
    if param_dict['dataset'] == 'bj_taxi':
        param_dict['train_data'] = [data[name] for name in ['t_vacation_train', 't_hour_train', 't_dayOfWeek_train',
                                't_weather_train', 't_continuous_external_train', 'stack_local_flow_train']]
        param_dict['test_data'] = [data[name] for name in ['t_vacation_test', 't_hour_test', 't_dayOfWeek_test',
                               't_weather_test', 't_continuous_external_test', 'stack_local_flow_test']]
    else:
