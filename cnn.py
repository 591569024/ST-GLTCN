...
parser.add_argument('--len_local', type=int, default=5, help='the length of local time')
parser.add_argument('--neighbor_size', type=int, default=3, help='the size of neighbor size')
parser.add_argument('--cnn', type=int, default=1, help='how many cnn unit')
...
# file params
param_dict['header'] = {'len_local': 10, 'neighbor_size': 15, 'cnn': 5, 'lr': 8, 'BN': 5,
                        'start_time': 10, 'end_time': 10, 'time': 20, 'rmse':8, 'mape': 8}
param_dict['hyper_params'] = {'len_local': len_local, 'neighbor_size': neighbor_size,
                             'cnn': args.cnn, 'lr': args.lr, 'BN': True if args.BN == 1 else False}
