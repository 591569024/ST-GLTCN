...
parser.add_argument('--len_global', type=int, default=1, help='the length of global time')
...
parser.add_argument('--res', type=int, default=8, help='how many Sres unit')
parser.add_argument('--cnn', type=int, default=6, help='how many cnn unit')
parser.add_argument('--tcn', type=int, default=2, help='how many tcn unit')
parser.add_argument('--nb_stacks', type=int, default=2, help='how many tcn unit')
...
param_dict['len_global'] = args.len_global
param_dict['len_local'] = args.len_local
param_dict['neighbor_size'] = args.neighbor_size

# model params
param_dict['model'] = ['TCN', 'tcn',
                       {'len_global': args.len_global, 'len_local': args.len_local, 'neighbor_size': args.neighbor_size,
                        'is_BN': True if args.BN == 1 else False, 'nb_res_unit': args.res,
                        'num_cnn': args.cnn, 'num_tcn': args.tcn, 'nb_stacks': args.nb_stacks,
...
param_dict['header'] = {'len_global': 10, 'len_local': 10, 'neighbor_size': 15, 'res': 5, 'cnn': 5, 'tcn': 5,
                        'nb_stacks': 10, 'lr': 8, 'BN': 5, 'start_time': 10, 'end_time': 10, 'time': 20, 'rmse':8, 'mape': 8}
param_dict['hyper_params'] = {'len_global': args.len_global, 'len_local': args.len_local, 'neighbor_size': args.neighbor_size,
                             'res': args.res, 'cnn': args.cnn, 'tcn': args.tcn, 'nb_stacks': args.nb_stacks,
                              'lr': args.lr, 'BN': True if args.BN == 1 else False}
